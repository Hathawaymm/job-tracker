...
| 21 | 经历拖拽排序 | 简历页工作/项目经历 + 经历库页工作/项目经历支持拖拽调换顺序（教育/基本信息不做）。原生 HTML5 DnD 零依赖：`useDragSort` hook（`client/src/hooks/useDragSort.ts`，把手 draggable + 卡片 drop，按鼠标上下半区判 before/after）+ `moveItem` 纯函数（`client/src/lib/reorder.ts`）。简历页排序走现有 onChange 自动入库；经历库需持久化：`experience_bank` 加 `sort_order` 列（`ensureColumn` 迁移 + `initExperienceSortOrder` 按旧顺序幂等初始化），`listExperiences` 改 `ORDER BY sort_order ASC, id ASC`，新增 `reorderExperiences(type, ids)` + `POST /api/experiences/reorder`，前端乐观更新 + 失败回滚。坑：dragId 用 ref 同步存（React 批处理异步导致同步 tick 内 drop 读不到）；经历库本地重排要按 ordered 顺序填回原索引而非按 id 换对象 | reorder 9 单测 + typecheck + 浏览器实测（简历页拖拽自动入库；经历库拖拽持久化 + 刷新保持） |
| 22 | 拖拽体验优化 | ① 经历库页项目经历去掉「时间」「标签」字段（ItemCard + AddForm + EMPTY_FORM + formToInput；`normalizeInput` 保留 tags 兼容旧 JSON 导入；库中已有 period/tags 数据保留，AI 匹配仍读标签）——简历页项目经历本就无这两个字段，不动；② 拖拽把手统一移到卡片操作区最右（简历页工作把手移到删除后、项目把手移到 STAR 润色后；经历库 `.card-grip` 从左侧移到 ItemCard 之后）；③ 紧凑化布局：`.card` padding 14→10、`.row` gap 12→8、`.field` margin 12→8、label margin 4→2、input/textarea padding 8→6、textarea min-height 80→56、`.ef-view` min-height 30→22 | typecheck + 110 测试 + 浏览器实测（项目卡无时间/标签、把手在最右 draggable、工作经历时间保留） |
